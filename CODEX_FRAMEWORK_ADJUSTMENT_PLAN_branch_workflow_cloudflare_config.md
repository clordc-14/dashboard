...
```text
admin   管理员：可以管理用户、文件夹、文件、所有数据、删除记录
editor  编辑者：可以新建/重命名文件夹、上传和新增文件版本、导入和更新解析后的数据；不能管理用户或删除记录
viewer  查看者：只能查看已获授权的首页、表格、搜索、分析结果和文件元数据
```
...
2. 支持 .xlsx / .xls / .docx / .pptx。
3. 原始文件保存到 R2。
4. 文件元数据保存到 D1 的 files / file_versions 表。
5. 资料库页面支持上传文件到当前文件夹。
6. 文件列表展示 name、size、ext、status、updated_at。
7. 所有文件 API 复用 RBAC：viewer 只读，editor/admin 上传和新版本，只有 admin 归档/删除；操作者写入 metadata/audit。
```
...
7. POST /api/dashboard/import 只允许 editor/admin，并为导入写入操作者审计；viewer 必须得到 403。
```
...
4. 资料库页面展示“已提取文本 / 未提取文本 / 提取失败”。
5. 搜索接口可以搜索 document_chunks.content。
```
