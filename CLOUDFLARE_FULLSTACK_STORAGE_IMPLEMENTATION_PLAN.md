...
## 0. Approved Library And File-Use Policy (2026-08-13 Revision)

This section is the latest constraint on every earlier statement in this plan. Where an earlier role, file type, file-access, or Phase 4/5 statement conflicts with it, this section wins. It does not authorize execution of a later phase without a separate user request.

1. Every uploaded source file remains in private R2 and **no role may download it**. Do not add a download endpoint, public URL, presigned URL, or return an R2 key to the browser. Exporting a rendered report page as PDF is not a file download.
2. The library, its folders/files/versions/archive records, and future AI-material APIs are for admin and editor only. Viewer must not see a library entry; direct library-page, `/api/folders*`, `/api/files*`, or material-processing requests must be rejected. Viewer may access report-dashboard pages only.
3. Excel (`.xlsx`, `.xls`) is the only dashboard data source. In a later parse/publish flow, admin validates and publishes the structured Excel result; editor retains existing upload/new-version rights but may not publish global dashboard data.
...
admin: manage user roles, all folders/files, Excel parse/publication, online edits, and destructive actions.
editor: create/rename folders and upload/version files; cannot manage roles, archive/delete files, or publish dashboard data.
viewer: read published reports, tables, search, and analysis results and export the current page as PDF only; cannot access the library or file metadata.
```
...
```text
viewer: 403; may not read library folders
...
```text
请阅读 CLOUDFLARE_FULLSTACK_STORAGE_IMPLEMENTATION_PLAN.md 和 D:\报表展示页面\CODEX_FRAMEWORK_ADJUSTMENT_PLAN_branch_workflow_cloudflare_config.md，然后只执行 Phase 0。不要进入 Phase 1。执行前先检查 git status；如果工作区不干净，先停止并说明风险。
```

### Execute Phase 1 Only

```text
请阅读 CLOUDFLARE_FULLSTACK_STORAGE_IMPLEMENTATION_PLAN.md 和 D:\报表展示页面\CODEX_FRAMEWORK_ADJUSTMENT_PLAN_branch_workflow_cloudflare_config.md，然后在当前 feature 分支上只执行 Phase 1。不要改现有分析页面逻辑。完成后运行 npm run build，并输出 Cloudflare 控制台待办、Codex 自检结果和用户本地实机验证清单。
```
