...
只执行 Phase 3。禁止进入或提前实现 Phase 4 及以后内容：远程 dashboardState 导入/读取、页面数据源切换、IndexedDB/demo fallback 改动、在线编辑、Word/PPT 解析、搜索/Vectorize/AI 分析，或重写任何现有报表页。
...
viewer 可读取列表和详情；viewer 上传、上传版本、归档均为 403。
...
写 R2 前服务端验证：multipart content type；file 存在且为 File；folderId 语法及 folder 存在；扩展名；大小；安全文件名；当前权限；同源请求。只允许 .xlsx、.xls、.docx、.pptx（大小写无关）。MAX_UPLOAD_BYTES 默认 20971520。服务器根据允许扩展名映射 MIME，绝不信任浏览器 file.type。客户端可以做 UX 校验，但服务端必须独立验证。
...
在现有资料库 UI 上增量实现：当前 folder 文件列表显示 name、size、ext、status、updatedAt；admin/editor 在非根目录可上传；viewer 不显示上传；上传成功刷新列表；失败给安全清晰提示；admin 可归档；admin/editor 可新增有效文件版本；默认不显示 archived；不实现下载、公开链接、Office 预览或解析。
...
运行已有相关测试（如有），不为形式引入大型 test framework。检查：文件列表；admin/editor 上传；viewer 上传 403；admin/editor 新版本；viewer 新版本 403；admin archive + audit；editor/viewer archive 403；缺失文件、非法 folder、非法扩展名、超限文件；R2 成功/D1 失败 cleanup code path。明确区分实际执行和仅代码路径审查。
